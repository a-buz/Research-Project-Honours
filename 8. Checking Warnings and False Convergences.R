...
convergenceCheck <- function(model) {
  # Fitted models
  fittedModels <- list.dirs(paste0('Data/', model))
  # Remove base dir
  ...
  for (i in 1:nrow(messages)) {
    for(j in 2:length(colnames(messages))) {
      load(paste0('Data/', model, '/', colnames(messages)[j], '/', messages$Number[i], '.Rdata'))
  ...
  # Write
  write_csv(converged, paste0('Data/', model,'/ConvergenceChecked.csv'))
}

convergenceCheck('GR4J')
convergenceCheck('SIMHYD')
convergenceCheck('HBV')
